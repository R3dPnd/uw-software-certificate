...
def print_armstrong_numbers(numbers, process_id):
    print(f"Process {process_id} found Armstrong numbers: {numbers}")
...
def main():
    ...
    if args.number is None:
        number_limit = int(input("Enter the upper limit for Armstrong number search: "))
    else:
    ...
    # Sort and print all Armstrong numbers
    all_armstrong_numbers.sort()
    print("\nAll Armstrong numbers found (in order):")
    print(all_armstrong_numbers)

    # Calculate and print runtime
    end_time = time.time()
    runtime = (end_time - start_time) * 1000  # Convert to milliseconds
    print(f"\nRuntime: {runtime:.2f} milliseconds")
