...
def closest_pair_dc(p):
    ...
    # Find the mid point and recursively find the closest pair in the left and right halves
    mid = len(p) // 2
    dl = closest_pair_dc(p[:mid])
    dr = closest_pair_dc(p[mid:])

    # Find the minimum distance and the points in the left and right halves
     # and set the minimum distance to the smaller of the two 
    if dl[0] < dr[0]:
        d = dl[0]
        min_points = dl[1]
    else:
    ...
    # Find the points with in d distance from m
    S = []
    for point in p:
        if abs(point[0] - m) < d:
            S.append(point)

    # Sort S by Y so we can start from the bottom
    S.sort(key=lambda y: y[1])
    
    min_dist = d
    
    # Compare each point in S with the next 7 points
